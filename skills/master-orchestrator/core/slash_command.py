...
class PromptCommandHandler(SlashCommandHandler):
    """Handler for prompt template commands (/template-*, /render-*, etc.)."""

    def execute(self, command: SlashCommandMetadata, args: List[str],
                kwargs: Dict[str, Any]) -> SlashCommandResult:
        """Execute prompt template command."""
        start_time = datetime.now()

        try:
            # Get template name
            template_name = command.prompt_template
            if not template_name:
                raise ValueError(f"Prompt command '{command.name}' has no template specified")

            # Parse variables from args and kwargs
            # Format: /template-name var1=value1 var2=value2
            variables = self._parse_variables(args, kwargs)

            # Execute via PromptManager
            ...
            manager = PromptManager(
                backend_orch=self.orchestrator.backend_orch,
                use_claude_renderer=True,
                fallback_to_local=True
            )

            # Render the template
            result = manager.render(template_name, **variables)
            ...
            output = f"Template: {result.template_name}\n"
            output += f"Rendered by: {result.rendered_by}\n"
            output += f"Variables: {result.variables}\n"
            ...
            return SlashCommandResult(
                ...
                metadata={
                    ...
                    'rendered_by': result.rendered_by,
            ...
            duration = (datetime.now() - start_time).total_seconds()
            logger.error(f"Prompt command '{command.name}' failed: {e}")
    ...
    def _parse_variables(self, args: List[str], kwargs: Dict[str, Any]) -> Dict[str, Any]:
        """
        Parse template variables from args and kwargs.

        Args:
            args: List of arguments (e.g., ["key1=value1", "key2=value2"])
        ...
        for arg in args:
            if '=' in arg:
                ...
                # If no '=', treat as positional variable (e.g., code, language)
                # This is a simple heuristic; can be improved
                if 'code' not in variables:
                    variables['code'] = arg
                elif 'language' not in variables:
                    variables['language'] = arg
...
HANDLER_MAP = {
    ...
    SlashCommandType.PROMPT: PromptCommandHandler,
}
