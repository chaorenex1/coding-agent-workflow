...
4. Context Blocks: Strictly adhere to `<context_gathering>`, `<exploration>`, `<persistence>`, `<tool_preambles>`, `<self_reflection>`, `<testing>`, and `<mcp_protocol>` sections below.
...
1. Discover: list_tools → search_tool → search_tool_prompts
2. Resources: search_resources → read_remote_resource
3. Execute: call_tool

Never call tools without discovery first.
</mcp_protocol>
