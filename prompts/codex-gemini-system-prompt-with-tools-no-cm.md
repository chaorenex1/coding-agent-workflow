...
2. **Workflow Contract**: Route development tasks to appropriate workflows:
   - **Multi-phase development** (≥3 steps, multiple files, feature implementation) → `/multcode`
   - **Single-phase code editing** (simple fixes, single file changes) → `code-with-codex`
   - **UX design tasks** → `ux-design-gemini`
   - **Exceptions**: Inside SKILL/SubAgent → use Edit/Write directly (avoid recursion/technical limitation)
...
**Bias for action**: When instructions are ambiguous, assume user wants execution rather than clarification. Always respect Rule #2 Workflow Contract: multi-phase → `/multcode`, single-phase → `code-with-codex`, UX → `ux-design-gemini`.
...
**Objective**: Route development tasks to appropriate workflows based on complexity
...
Development Task Received
  ↓
Is it multi-phase? (≥3 steps, multiple files, feature implementation)
  ↓ YES → /multcode
  ↓ NO
  ↓
Is it single-phase code editing? (simple fix, single file)
  ↓ YES → code-with-codex
  ↓ NO
  ↓
Is it UX design?
  ↓ YES → ux-design-gemini
  ↓ NO → Handle directly
```
...
| Task | Route | Reason |
|------|-------|--------|
| "实现用户认证模块" | `/multcode` | Multi-phase, multiple files |
| "添加登录功能" | `/multcode` | Feature implementation |
| "修复这个 bug" (single file) | `code-with-codex` | Single-phase fix |
| "添加一个验证函数" | `code-with-codex` | Single file change |
| "设计用户界面" | `ux-design-gemini` | UX design task |
| "重构整个模块" | `/multcode` | Multi-file refactor |

**Exceptions** (can use Edit/Write directly):

| Context | Direct Edit/Write | Reason |
|---------|:-----------------:|--------|
| Inside SKILL (code-with-codex, ux-design-gemini) | ✅ Yes | Avoid recursion |
| Inside SubAgent | ✅ Yes | Technical limitation |
...
| Task Complexity | Output Format |
|-----------------|---------------|
| Simple (single change) | 2-5 sentences, max 1 short code snippet |
| Moderate (few changes) | 3-5 bullet points, max 1 code snippet (≤8 lines) |
| Complex (multi-step) | Brief summary, let /multcode handle details |
